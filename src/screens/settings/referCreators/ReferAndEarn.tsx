...
import { getReferralLinks } from "@helper/endpoints/profile/apis";
import { CreatorReferral } from "@usertypes/types";
import { setStringAsync } from "expo-clipboard";

const ReferCreatorsEarnScreen = () =>
	// props: ReferralProgramNativeStackScreenProps<"ReferCreators">,
	{
		const navigation =
			useNavigation<
		...
		const handlePressAnalytics = () =>
			router.push({
				pathname: "/refer-analytics",
				params: { screen: "ReferralAnalytics" },
			});
		...
		const handlePressPayout = () =>
			router.push({
				pathname: "/refer-payout",
				params: { screen: "ReferralPayments" },
			});

		const [selectedReferral, setSelectedReferral] =
			useState<CreatorReferral>();
		...
		const getLinkCode = (index: number) => {
			return `fyp.fans/?r=${links[index].code ?? ""}`;
		};
			...
			<FansScreen3 contentStyle={tw.style("p-[0px]")}>
				...
				>
					...
					<FansView alignItems="center">
						<FansView
							style={tw.style("max-w-[306px] lg:max-w-full")}
						>
							<FansText
								color="white"
								fontFamily="inter-bold"
						...
						<FansView style={tw.style("max-w-[319px]")}>
							<FansText
								...
								textAlign="center"
							>
								Earn LIFETIME 10% on creators income when they
						...
						<FansView>
							<FansImage2
								...
								height={{
									xs: (dimensions.width - 36) * 0.65,
									lg: 436,
								}}
								source={require("@assets/images/refer-creators-image.png")}
								viewStyle={{ borderRadius: 7 }}
							/>
							<FansIconButton
								size={84}
								containerColor="#ffffff7f"
								style={{
									position: "absolute",
									left: "50%",
									top: "50%",
					...
					>
						...
						<FansGap height={15} />
						<FansView
							style={tw.style("max-w-[255px] lg:max-w-none")}
						>
					...
					<FansView
						style={tw.style(
							"rounded-[15px] border-[2px] border-fans-grey",
						)}
						alignItems="center"
						overflow="hidden"
					>
						...
						>
							...
							>
								...
								today and set foot on the path to financial
								freedom. Your success story is waiting to be
								written!
							</FansText>
						</FansView>
						<FansGap height={26} />
								...
								<FansView flexDirection="row">
									...
									>
										...
										<FansText
											...
											fontSize={16}
										>
											Check our blog with posts about how
											to market creators
										</FansText>
									</FansView>
								</FansView>
							...
							<FansView alignItems="center">
								...
								<FansView style={tw.style("max-w-[320px]")}>
									...
									>
										...
										with a 20% share from this referral
									</FansText>
								</FansView>
							</FansView>
							{/* ~ Lifetime passive income */}
							...
							<FansView alignItems="center">
								...
								<FansView style={tw.style("max-w-[276px]")}>
									...
									>
										...
										. Your earnings, in your pocket, right
										away
									</FansText>
								</FansView>
							</FansView>
							...
							<FansView alignItems="center">
								...
								<FansView style={tw.style("max-w-[314px]")}>
									<FansText
										...
										textAlign="center"
									>
										Harness the power of data. Dive into
					...
					<FansView
						style={tw.style(
							"mt-[40px] bg-fans-white px-[17px] py-[22px]  shadow-lg rounded-[15px] bg-[#5F17D3]",
							"w-full max-w-[673px]",
						)}
					>
						{/* Your referral link ~ */}
